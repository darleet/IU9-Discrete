...
func printGraph(mealy *machine, writer *bufio.Writer) {
	...
	for i := 0; i < mealy.qNum; i++ {
		for j := 0; j < mealy.xNum; j++ {
			...
			writer.WriteString(fmt.Sprintf("[label = \"%c(%s)\"]\n", rune('a'+j), mealy.fmatrix[i][j]))
		}
	}
	writer.WriteString("}\n")
}

func readMachine(scanner *bufio.Scanner, q0, n, m int) *machine {
	dmatrix := make([][]int, n)
	for i := 0; i < n; i++ {
...
func areEqual(mealy1, mealy2 *machine) bool {
	if mealy1.qNum != mealy2.qNum {
		return false
	}
	if mealy1.qStart != mealy2.qStart {
		return false
	}
	if mealy1.xNum != mealy2.xNum {
		return false
	}

	for i := 0; i < mealy1.qNum; i++ {
		for j := 0; j < mealy1.xNum; j++ {
			if mealy1.dmatrix[i][j] != mealy2.dmatrix[i][j] ||
				mealy1.fmatrix[i][j] != mealy2.fmatrix[i][j] {
				return false
			}
		}
	}
...
func main() {
	...
	mealy1 := readMachine(scanner, q0, n, m)
	...
	scanner.Scan()
	q0, _ = strconv.Atoi(scanner.Text())
	mealy2 := readMachine(scanner, q0, n, m)

	calcCanonic(mealy1)
	mealy11 := minimize(mealy1)
	calcCanonic(mealy11)

	calcCanonic(mealy2)
	mealy21 := minimize(mealy2)
	calcCanonic(mealy21)

	if areEqual(mealy11, mealy21) {
		writer.WriteString("EQUAL\n")
	} else {
		writer.WriteString("NOT EQUAL\n")
	}
}
